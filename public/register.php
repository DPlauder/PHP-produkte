...
if($_SERVER['REQUEST_METHOD'] === 'POST'){
    ...
    $errors['password'] = Validate::is_password($user['password']) ? '' : 'Password must be at least 8 characters long and contain at least one uppercase letter, one lowercase letter and one number';
    $errors['confirm'] = ($user['password'] === $confirm) ? '' : 'Password do not match';

    $problems = implode($errors);

    if( empty($problems) ){
        if($cms->getUser()->addUser($user)){
            redirect('login.php', ['success' => 'User added successfull']);
        }
        else{
